...
public static class DebugExtension
{
	...
	public static void DebugColliders(Collider2D[] colliders, Vector2 positionOffset, Color color, float duration = 0.0f)
	{
		for (int i = 0; i < colliders.Length; ++i)
		{
			...
			if (colliders[i] is CircleCollider2D)
			{
				...
				DebugCircle(collider.bounds.center + (Vector3)positionOffset, Vector3.forward, color, collider.radius, duration);
				continue;
			}
		}
	}
	public static void DebugColliders(List<Collider2D> colliders, Vector2 positionOffset, Color color, float duration = 0.0f)
	{
		DebugColliders(colliders.ToArray(), positionOffset, color, duration);
	}
