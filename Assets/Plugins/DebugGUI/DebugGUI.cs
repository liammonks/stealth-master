...
public class DebugGUI : MonoBehaviour
{
    ...
    public static void Show()
    {
        ...
        _instance.displayGraphs = true;
        _instance.displayLogs = true;
    }

    public static void Hide()
    {
        if (_instance == null) { return; }
        _instance.displayGraphs = false;
        _instance.displayLogs = false;
    }
    ...
    void OnGUI()
    {
        ...
        if (LogsEnabled && persistentLogs.Count > 0)
            DrawLogs();
    ...
    void DrawGraphs()
    {
        ...
        var mousePos = m_MouseScreenPosition;
        mousePos.y = Screen.height - m_MouseScreenPosition.y;

        if (freezeGraphs && !m_MouseDown)
            freezeGraphs = false;
    ...
    class GraphContainer
    {
        ...
        public void Push(float val)
        {
            if (autoScale)
            {
                ...
                {
                    valuesMin = Mathf.Min(valuesMin, entry);
                    valuesMax = Mathf.Max(valuesMax, entry);
                }
                SetMinMax(valuesMin, valuesMax);
            }
    ...
    {
        m_MouseDown = value.Get<float>() == 1.0f;
    }
