...
{
    private const string levelPath = "Assets/Gameplay/Levels/";
    private Vector2 scrollPos;

    /// <summary>
    /// Initialize window state.
    /// </summary>
    [MenuItem("Stealth Master/Levels Window")]
    internal static void Init()
    {
        ...
        var window = (SceneSwitchWindow)GetWindow(typeof(SceneSwitchWindow), false, "Levels");
    ...
    internal void OnGUI()
    {
        ...
        for (var i = 0; i < EditorBuildSettings.scenes.Length; i++)
        {
            ...
            if (scene.enabled && scene.path.Contains(levelPath))
            {
                var sceneName = Path.GetFileNameWithoutExtension(scene.path);
                var pressed = GUILayout.Button(sceneName.Replace("Level_", ""), new GUIStyle(GUI.skin.GetStyle("Button")) { alignment = TextAnchor.MiddleLeft });
                if (pressed)
                {
